...
import spring.boot.handle.LoginSuccessHandler;
import spring.boot.service.CustomUserDetailsService;
@Configuration
@EnableWebSecurity
...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		.defaultSuccessUrl("/hello")//登录成功之后默认跳转页面
		.failureUrl("/login?error")//登录错误显示
		.permitAll()
		.successHandler(loginSuccessHandler()) //登录成功后可使用loginSuccessHandler()存储用户信息，可选。
		.and()
		.logout()
		.logoutSuccessUrl("/login?logout") //退出登录后的默认网址是”/login”
		.permitAll()
